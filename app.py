...
def facet_chips(observed: list[str], expected: list[str]) -> str:
    chips = [f"`{v}`" for v in observed] + [f"⬜ `{v}`" for v in expected if v not in observed]
    ...
    if obs:
        ...
        for ob in obs:
            with st.container(border=True):
                ...
                with c1:
                    st.markdown(f"`{ob['id']}`" + ("  :gray[(unverified)]" if ob.get("verified") is False else ""))
                    if ob.get("task"):
                ...
                with c2:
                    ...
                    st.markdown(OUTCOME_BADGE.get(ob.get("outcome"), ob.get("outcome", "—")))
                    st.markdown(f"Source: {SOURCE_BADGE.get(ob.get('source'), ob.get('source', '—'))}")
                    if ob.get("route"):
...
def main() -> None:
    st.set_page_config(page_title="GUI Failure Taxonomy (v2)", page_icon="🔍", layout="wide")

    st.title("GUI Agent Failure Taxonomy — v2")
    st.caption(
    ...
    with st.sidebar:
        ...
        all_categories = sorted({t["category"] for t in types})
        all_causes = sorted({t.get("facets", {}).get("cause") for t in types if t.get("facets", {}).get("cause")})
        all_stages = sorted({t.get("facets", {}).get("stage") for t in types if t.get("facets", {}).get("stage")})
        all_sources = sorted({ob.get("source") for t in types for ob in t.get("observations", []) if ob.get("source")})
        all_outcomes = sorted({ob.get("outcome") for t in types for ob in t.get("observations", []) if ob.get("outcome")})
        ...
        st.divider()
        sel_reps = st.multiselect("Representation", REPRESENTATIONS, default=REPRESENTATIONS)
        ...
        st.divider()
        sel_sources = st.multiselect("Observation source", all_sources, default=all_sources)
        sel_outcomes = st.multiselect("Observation outcome", all_outcomes, default=all_outcomes)

        st.divider()
        search = st.text_input("Search", placeholder="failure name, description, or ID…")

    def type_matches(t: dict) -> bool:
        if t["_source_file"] not in sel_files:
            return False
        if t["category"] not in sel_categories:
            return False
        ...
        plat_obs, plat_exp = facet_values(t, "platform")
        plat_pool = plat_obs if observed_only else set(plat_obs) | set(plat_exp)
        ...
        if q:
            haystack = " ".join(
                [t.get("id", ""), t.get("failure", ""), t.get("description", "")]
            ).lower()
    ...
    m2.metric("Observations", sum(len(t.get("observations", [])) for t in filtered))
    m3.metric("With observations", sum(1 for t in filtered if t.get("observations")))
    ...
    for t in filtered:
        category_label = CATEGORY_LABELS.get(t["category"], t["category"])
        n_obs = len(t.get("observations", []))
        badge = f"({n_obs} obs.)" if n_obs else "(predicted only)"

        with st.expander(
            f"{t['id']}  ·  {t['failure'].replace('_', ' ')}  ·  {category_label}  ·  "
